# STACKS & QUEUES資料區 

本資料夾收錄 CS203A 課程中與 Stack（堆疊）與 Queue（佇列）相關的學習筆記與程式範例，涵蓋基本操作、實作方式與效能比較。

##  檔案一覽

| 檔案名稱                     | 說明                                   |
|------------------------------|----------------------------------------|
| `Stack`                      | Stack 的基本概念與操作實作             |
| `Queue`                      | Queue 的基本概念與操作實作             |
| `Using Array to Implement`   | 使用陣列實作 Stack 與 Queue            |
| `Using Linked List to implement` | 使用 Linked List 實作 Stack 與 Queue |
| `實作比較`                   | 陣列與鏈結串列實作方式的比較分析       |
| `Array vs. Linked List`      | 資料結構選擇的概念比較                 |
| `講義P1-P13`                 | 課堂講義內容（第 1 至 13 頁）          |
| `Bonus`                      | 進階挑戰內容與延伸思考                   |

##  學習目標

- 理解 Stack 與 Queue 的基本結構與操作邏輯
- 掌握先進先出（FIFO）與後進先出（LIFO）的應用情境
- 熟悉使用陣列與鏈結串列實作 Stack/Queue 的差異
- 強化資料結構選擇與效能分析的能力
- 掌握常見操作的時間與空間複雜度（Big O）

##  Big O 快速查詢

### Stack 操作效率

| 操作類型     | 時間複雜度 | 空間複雜度 | 備註                   |
|--------------|------------|------------|------------------------|
| push（加入） | O(1)       | O(1)       | 加入至頂端             |
| pop（移除）  | O(1)       | O(1)       | 移除頂端元素           |
| peek（查看） | O(1)       | O(1)       | 查看頂端但不移除       |

### Queue 操作效率

| 操作類型     | 時間複雜度 | 空間複雜度 | 備註                   |
|--------------|------------|------------|------------------------|
| enqueue（加入） | O(1)    | O(1)       | 加入至尾端             |
| dequeue（移除） | O(1) 或 O(n) | O(1)   | 若無 tail 指標則需遍歷 |
| peek（查看） | O(1)       | O(1)       | 查看前端但不移除       |

> 註：若使用陣列實作，Queue 的 dequeue 操作可能需移動元素，導致 O(n) 時間複雜度；使用 Linked List 可達 O(1)。
